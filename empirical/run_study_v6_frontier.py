#!/usr/bin/env python3
"""
Em Dash Frequency Study v6 — Frontier Models
==============================================
Tests the most capable current models from each provider.
...
OUTPUT_DIR = Path(__file__).parent / "samples_v6"
RESULTS_FILE = Path(__file__).parent / "results_v6.json"
...
def register_models():
    if os.environ.get("ANTHROPIC_API_KEY"):
        MODELS.append(("Claude Opus 4.6", "claude-opus-4-6", call_anthropic))
    if os.environ.get("OPENAI_API_KEY"):
        MODELS.append(("GPT-5.4", "gpt-5.4", call_openai))
        MODELS.append(("GPT-4.1", "gpt-4.1", call_openai))
...
def main():
    ...
    print(f"\n{'='*70}")
    print(f"  FRONTIER MODEL RESULTS")
    print(f"{'='*70}")
